...
import { chatModificationToAppPatch, decodePatches, decodeSyncdSnapshot, encodeSyncdPatch, ensureLTHashStateVersion, extractSyncdPatches, generateProfilePicture, getContentType, getHistoryMsg, isAppStateSyncIrrecoverable, isMissingKeyError, MAX_SYNC_ATTEMPTS, newLTHashState, normalizeMessageContent, processSyncAction } from '../Utils/index.js';
import { makeMutex } from '../Utils/make-mutex.js';
import processMessage from '../Utils/process-message.js';
import { buildTcTokenFromJid } from '../Utils/tc-token-utils.js';
import { getBinaryNodeChild, getBinaryNodeChildren, isHostedLidUser, isHostedPnUser, isLidUser, isPnUser, jidDecode, jidNormalizedUser, lidToJid, reduceBinaryNodeToDictionary, S_WHATSAPP_NET } from '../WABinary/index.js';
...
export const makeChatsSocket = (config) => {
    ...
    const receivedMessageDedupCache = new NodeCache({
    ...
    const extractStanzaId = (msg) => {
        const topLevelStanzaId = msg.messageContextInfo?.stanzaId;
        if (topLevelStanzaId) {
            return topLevelStanzaId;
        }
        const messageContextStanzaId = msg.message?.messageContextInfo;
        ...
        const contentContextStanzaId = contentType
            ? normalizedContent[contentType]?.contextInfo?.stanzaId
            : undefined;
    ...
    const getDedupState = (msg) => msg.messageStubType === proto.WebMessageInfo.StubType.CIPHERTEXT
        ? 'ciphertext'
        : 'final';
    const upsertMessage = ev.createBufferedFunction(async (msg, type, extraEventData) => {
