...
import { areJidsSameUser, binaryNodeToString, getAllBinaryNodeChildren, getBinaryNodeChild, getBinaryNodeChildBuffer, getBinaryNodeChildren, getBinaryNodeChildString, getBinaryNodeChildUInt, getLidForPn, isJidGroup, isJidNewsletter, isJidStatusBroadcast, isLidUser, isPnUser, jidDecode, jidNormalizedUser, S_WHATSAPP_NET, storeLidPnMapping } from '../WABinary/index.js';
...
export const makeMessagesRecvSocket = (config) => {
    ...
    const requestPlaceholderResend = async (messageKey, msgData) => {
        ...
        setTimeout(async () => {
            if (await placeholderResendCache.get(messageKey?.id)) {
                logger.debug({ messageKey }, 'PDO message without response after 5 seconds. Phone possibly offline');
        ...
        for (const tokenNode of tokenNodes) {
            if (tokenNode.attrs.type === 'trusted_contact') {
                ev.emit('chats.update', [
                    {
                        id: tokenNode.attrs.jid,
                        tcToken: tokenNode.content
                    }
                ]);
            }
        }
                ...
                if (msg.messageStubType === proto.WebMessageInfo.StubType.CIPHERTEXT && msg.category !== 'peer') {
                    ...
                    if (!isNewsletter) {
                        if (!isJidGroup(msg.key.remoteJid) && isLidUser(msg.key.remoteJid)) {
                            const targetJid = node.attrs.sender_pn || node.attrs.peer_recipient_pn;
                            if (targetJid) {
                                storeLidPnMapping(msg.key.remoteJid, targetJid);
