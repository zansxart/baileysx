...
import { aggregateMessageKeysNotFromMe, assertMediaContent, assertMeId, bindWaitForEvent, decryptMediaRetryData, DEF_MEDIA_HOST, encodeNewsletterMessage, encodeSignedDeviceIdentity, encodeWAMessage, encryptMediaRetryRequest, extractDeviceJids, generateMessageIDV2, generateParticipantHashV2, generateWAMessage, generateWAMessageContent, generateWAMessageFromContent, getContentType, getStatusCodeForMediaRetry, getUrlFromDirectPath, getWAUploadToServer, isPrivateChat, MessageRetryManager, normalizeMessageContent, parseAndInjectE2ESessions, unixTimestampSeconds } from '../Utils/index.js';
...
import { areJidsSameUser, getBinaryNodeChild, getBinaryNodeChildren, getLidForPn, isHostedLidUser, isHostedPnUser, isJidBot, isJidGroup, isJidMetaAI, isLidUser, isPnUser, jidDecode, jidEncode, jidNormalizedUser, jidToLid, PSA_WID, S_WHATSAPP_NET } from '../WABinary/index.js';
...
export const makeMessagesSocket = (config) => {
    ...
    const sendReceipt = async (jid, participant, messageIds, type) => {
        ...
        if (participant) {
            ...
            participant = jidToLid(participant);
        }
    ...
    const relayMessage = async (jid, message, options) => {
        let { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList, ai } = options;
        ...
        if (participant && participant.jid) {
            ...
            participant.jid = jidToLid(participant.jid);
        }
        ...
        await authState.keys.transaction(async () => {
            ...
            if ((isJidGroup(jid) || isPnUser(jid) || isLidUser(jid)) && (contentType === 'interactiveMessage' ||
                contentType === 'buttonsMessage' ||
                contentType === 'listMessage')) {
                const bizNode = { tag: 'biz', attrs: {} };
                if (normContent?.interactiveMessage ||
                ...
                else if (normContent?.listMessage ||
                    message?.listMessage ||
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            const mentionAll = (typeof content === 'object' && 'mentionAll' in content && content.mentionAll) || options.mentionAll;
            ...
            if (typeof content === 'object' &&
                ...
                if (storyData.message) {
                    // Already a prepared message object
                ...
                const isExplicitAi = content?.ai !== undefined ? content.ai : options?.ai;
