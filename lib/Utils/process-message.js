...
export const cleanMessage = (message, meId, meLid, bypassViewOnce) => {
    ...
    if (bypassViewOnce) {
        if (message.message?.viewOnceMessage?.message) {
            ...
            if (type && message.message[type]) {
                message.message[type].viewOnce = false;
            }
        }
...
const processMessage = async (message, { shouldProcessHistoryMsg, placeholderResendCache, ev, creds, signalRepository, keyStore, logger, options, getMessage, upsertMessage, bypassViewOnce }) => {
    ...
    if (protocolMsg) {
                        ...
                        try {
                            ...
                            if (typeof upsertMessage === 'function') {
                                ...
                                Promise.resolve(upsertMessage(finalMsg, 'notify', {
                                    ...
                                    logger?.debug({ error, messageId: finalMsg?.key?.id }, 'failed to upsert placeholder resend response');
                                });
                            }
                            else {
        ...
        if (getMessage) {
            ...
            if (pollMsg) {
                ...
                try {
                                ...
                                pollUpdates: [
                                    {
                                        ...
                                        vote: voteMsg,
                                        senderTimestampMs: content.pollUpdateMessage.senderTimestampMs?.toNumber ? content.pollUpdateMessage.senderTimestampMs.toNumber() : Number(content.pollUpdateMessage.senderTimestampMs),
                                    }
                                ]
                            }
                        }
                    ...
                    ev.emit('polls.vote', {
                        ...
                        voter: voterJid,
                        vote: voteMsg,
                        timestamp: content.pollUpdateMessage.senderTimestampMs?.toNumber ? content.pollUpdateMessage.senderTimestampMs.toNumber() : Number(content.pollUpdateMessage.senderTimestampMs)
                    });
                }
                catch (err) {
            ...
            logger?.warn({ creationMsgKey }, 'getMessage not provided, cannot decrypt poll update');
        }
    }
